...
export default function ReachMetrics() {
  ...
  return (
    <section className="relative py-20 px-4 sm:px-6 overflow-x-hidden bg-background text-foreground transition-colors duration-300">

      {/* Top Purple Glow */}
      <div className="pointer-events-none absolute top-0 left-1/2 h-[280px] w-[600px] -translate-x-1/2 rounded-full bg-purple-700/50 dark:bg-purple-500/50 blur-[160px] transition-colors duration-300" />
      ...
      <div className="relative max-w-7xl mx-auto">
        ...
        <div className="relative mx-auto h-[520px] w-full lg:w-[900px] overflow-hidden rounded-2xl shadow-2xl">
          ...
          <div className="absolute inset-y-0 right-0 flex w-full flex-col items-start gap-4 p-6 sm:p-8 lg:w-[55%] lg:p-10 -translate-y-4">
            ...
            {/* Title & Subtitle */}
            <div>
              <h2 className="mb-3 text-3xl font-black text-black dark:text-white sm:text-4xl lg:text-5xl transition-colors duration-300">
                Our Reach So Far
              </h2>
              <p className="text-base text-black/80 dark:text-white/80 sm:text-lg leading-relaxed transition-colors duration-300">
                Here is how far we have gone on the mission to Educate & Entertain
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              {metrics.map((metric, index) => (
                <div
                  key={index}
                  className="rounded-2xl bg-purple-300 dark:bg-purple-800 px-4 py-3 text-center shadow-lg transition-colors duration-300 hover:shadow-xl"
                >
                  <div className="mb-1 text-xl font-black text-white dark:text-white lg:text-2xl transition-colors duration-300">
                    {metric.number}
                  </div>
                  <div className="mb-1 text-sm font-bold text-black dark:text-white lg:text-sm transition-colors duration-300">
                    {metric.label}
                  </div>
                  {metric.sublabel && (
                    <div className="text-xs text-black dark:text-white lg:text-xs transition-colors duration-300">
                      {metric.sublabel}
                    </div>
                  )}
                </div>
